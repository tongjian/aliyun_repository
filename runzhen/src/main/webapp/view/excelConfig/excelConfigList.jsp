	...
	<form id="excelConfigList_queryForm" role="form" class="form-horizontal" action="">
         <div class="form-group">
	         <div class="col-sm-4">
	            ...
	            <div class="col-sm-7">
	            	<input id="excelConfigList_queryParam_excelFileCode" name="excelFileCode" class="form-control" type="text" >
	            </div>
	         </div>
         </div>
	</form>
	...
	<table id="excelConfigList_table"
		...
		<thead>
		    <tr>
		        ...
		        <th data-field="dataFormat" data-align="center" data-sortable="true"
		        	data-formatter="dataFormatFormatter">数据格式</th>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-title">
                    <h2 id="excelConfigList_modal_title" class="text-center">用户编辑</h2>
                </div>
                <div class="modal-body">
                    <form id="excelConfigList_form" role="form" class="form-group form-horizontal" action="">
						<div class="form-group">
						    ...
						    <div class="col-sm-4">
						        <input id="excelConfigList_excelFileCode" name="excelFileCode" class="form-control" type="text" />
						    </div>
						    <label class="control-label col-sm-2">列索引:</label>
						    <div class="col-sm-4">
						        <input id="excelConfigList_userName" name="userName" class="form-control" type="text" />
						    </div>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
						    ...
						    <div class="col-sm-4">
						        <input id="excelConfigList_dataType" name="dataType" class="form-control" type="text" />
						    </div>
...
function excelConfigListOperateFormatter(value,row,index){
	...
	return '<button type="button" onclick="excelConfigList_editUser(\''+escape(jsonRow)+'\',\''+index+'\')" class="btn btn-primary btn-xs">修改</button>&nbsp;'+
			'<button type="button" onclick="excelConfigList_deleteUser('+value+')" class="btn btn-danger btn-xs">删除</button>';
}

function excelConfigList_editUser(jsonRow,index){
	var row = JSON.parse(unescape(jsonRow));		//先解码，再解析成json对象
	$("#excelConfigList_modal_title").text("修改用户");
	$("#excelConfigList_modal").modal('show');
	$("#excelConfigList_form")[0].reset();
	
	/* 初始化角色信息 */
	util_user_setFormData('excelConfigList',row);
}

//删除用户
function excelConfigList_deleteUser(id){
...
function excelConfigList_save(){
	...
	if(flag){
		util_form_save('excelConfigList','<%=contextPath%>/user/update',util_user_getFormData("excelConfigList"),'modal');
	}
}
